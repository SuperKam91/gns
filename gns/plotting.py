...
def cornerPlots(chainsFilePrefix, plotName, plotLegend, labels):
    ...
    for i, f in enumerate(chainsFilePrefix):
        # required to manually insert legend for histograms
...
def plotSphericalKDE(chains1, chains2=None):
    ...
    gs_up = matplotlib.gridspec.GridSpecFromSubplotSpec(
        1, 2, subplot_spec=gs_vert[0])
    gs_mid = matplotlib.gridspec.GridSpecFromSubplotSpec(
        1, 4, subplot_spec=gs_vert[1])
    gs_down = matplotlib.gridspec.GridSpecFromSubplotSpec(
        1, 2, subplot_spec=gs_vert[2])
