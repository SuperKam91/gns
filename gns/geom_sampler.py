...
def getShapeSigma(sigma, circleList, torusList, sphereList):
    ...
    """
    # assumes sigma is diagonal so that this returns a 1-d array of diagonal
    # elements
    sigmaArr = np.diag(sigma)
...
def getCartesianCoords(circleArr, torusArr, sphereArr, circleLowerLimits, circleUpperLimits,
    ...
    i = 0
    j = 0
    # for each circular parameter, convert to Cartesian coords and store all
    # of them in 1-d array
    while j < numCirc:
        circCartArr[i], circCartArr[i + 1] = geom.point2CartCirc(
            circleArr[j], circleLowerLimits[j], circleUpperLimits[j])
        i += 2
        j += 1
    i = 0
    j = 0
