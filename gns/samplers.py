...
def getNewLiveBlind(invPriorFunc, LhoodFunc, LhoodStar, nDims):
    ...
    while trialPointLhood <= LhoodStar:
        ...
        trialPointPhys = invPriorFunc(trialPoint)
        # calculate Lhood value of trialpoint
...
def getNewLiveMH(livePointsPhys, deadIndex, priorFunc, targetSupport,
    ...
    """
    # proposalType = 'truncated multivariate normal' #used to think this was
    # compulsory if using wrapped parameters. Not so sure anymore
    ...
    while nAccept == 0:
        ...
        while nTrials < maxTrials:
            ...
            # TODO: consider implementing function which gets non-geom
            # components of targetSupport and pass this instead of
            ...
            nonGeomSigma = geom_sampler.getNonGeomSigma(
                trialSigma, nonGeomList)
            circleSigma, torusSigma, sphereSigma = geom_sampler.getShapeSigma(
                trialSigma, circleList, torusList, sphereList)
            circCartSigArr, torusCartSigArr, sphereCartSigArr = geom_sampler.getCartesianSigma(
                circleStartPoint, torusStartPoint, sphereStartPoint,
...
def calcInitTrialSig(livePoints):
    ...
    trialSigma = np.sqrt(np.diag(0.1 * livePointsWidth))
    return trialSigma
