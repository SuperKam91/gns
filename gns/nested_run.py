...
def NestedRunLog(priorFunc, invPriorFunc, LLhoodFunc, paramNames,
    ...
    livePointsPhys = invPriorFunc(livePoints)
    ...
    nonGeomList, boundaryList, geomList, shapeList = geom_sampler.splitGeomParams(
        setupDict['paramGeomList'])
    circleList, torusList, sphereList = geom_sampler.splitGeomShapes(
        geomList, shapeList)
    nonGeomLowerLimits, nonGeomUpperLimits = geom_sampler.getNonGeomLimits(
        targetSupport, nonGeomList)
    circleLowerLimits, circleUpperLimits, torusLowerLimits, torusUpperLimits, sphereLowerLimits, sphereUpperLimits = geom_sampler.getShapeLimits(
        targetSupport, circleList, torusList, sphereList)
    while True:
    ...
    logEofZK, logEofZ2K = keeton_calculations.calcZMomentsKeetonLog(
        np.array(deadPointsLLhood), nLive, nest)
    ...
    logEofZFinalK, logEofZ2FinalK = keeton_calculations.calcZMomentsFinalKeetonLog(
        livePointsLLhood, nLive, nest)
...
def NestedRunLinear(priorFunc, invPriorFunc, LhoodFunc,
    ...
    # could also solve this by reshaping array in .pdf method of custom class
    livePointsLhood = array_checks.checkLhoodShape(livePointsLhood, nLive)
    # initialise lists for storing values
    EofXArr = []
    EofWeights = []
    deadPoints = []
    ...
    EofZK, EofZ2K = keeton_calculations.calcZMomentsKeeton(
        np.array(deadPointsLhood), nLive, nest)
    ...
    EofZFinalK, EofZ2FinalK = keeton_calculations.calcZMomentsFinalKeeton(
