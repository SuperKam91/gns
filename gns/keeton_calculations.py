...
def calcSumsLoop(Lhoods, nLive, nest):
    ...
    for k in range(1, nest + 1):
        ...
        for i in range(1, k + 1):
            innerSum += Lhoods[i - 1] * \
                calculations.Eoft2OverEoftPowi(nLive, i)
...
def calcHTotalKeeton(EofZ, Lhoods, nLive, nest, finalLhoods):
    ...
    # n.b. the 2nd contribution isn't HFinal, as we are dividing by EofZTotal
    # not EofZFinal
    return HPartial + 1. / EofZ * LAv * \
        np.log(LAv) * calculations.EoftPowi(nLive, nest)
...
def calcHTotalKeetonLog2(logEofZ, LLhoods, nLive, nest, finalLLhoods):
    ...
    logHPartial2 = - logEofZ + logFinalLhoodAv + \
        np.log(logFinalLhoodAv) + np.log(calculations.EoftPowi(nLive, nest))
