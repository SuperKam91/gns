...
def checkInputParamsShape(priorParams, LhoodParams, nDims):
    ...
    if LhoodParams[0] < 11:  # skip for kent sums or it's too complicated
        assert (
            LhoodParams[1].shape == (1, nDims) or LhoodParams[1].shape == (9, )
        ), "Llhood params mean array should have shape (1, nDims) or (9,) for Kent distriubtion"
        assert (
            LhoodParams[2].shape == (nDims, nDims) or
            LhoodParams[2].shape == (2, )
        ), "LLhood covariance array should have shape (nDims, nDims) or be (2,) for Kent distribution"
